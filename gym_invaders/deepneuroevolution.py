...
def return_random_agent(num_agents, input_shape, num_actions):
    ...
    for _ in range(num_agents):
        agent = EvolutionaryAI(input_shape,num_actions)
...
def run_agents(agents):
    ...
    for agent in agents:
        ...
        #Plays 1 game
        while True:
            input = torch.from_numpy(state).unsqueeze(0).to(device)
            output_probs = agent(input).detach().numpy()[0]
...
def survival_of_fittest(action_space, num_agents, input_dim, top_k, generations):
    ...
    for gen in range(generations):
        ...
        agents = child_agents
survival_of_fittest(game_actions,num_agents,input_shape,elites,generations)