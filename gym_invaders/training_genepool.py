...
def nn(obs, in_w, in_b, hid_w, out_w):
    ...
    print(out_put.shape)
    act = out_put[0][0] + (out_put[0][1] + 1)  # index of discrete action
...
def run_env(env, in_w, in_b, hid_w, out_w):
    """Run the environment until player dies"""
    obs = env.reset()
    award = 0
    while not env.is_over():
        # env.render(True)  # this slows the process
...
def reproduce(award_set, generations):
    ...
    for newdna in newDNA_list:  # collection of weights from dna info
        ...
        new_in_w = np.reshape(newdna_in_w1, (-1,
                              generations[0][0].shape[1]))
        new_input_weight.append(new_in_w)

        new_in_b = np.array([newdna[newdna_in_w1.size:newdna_in_w1.size
                            + generations[1][0].size]]).T  # bias
        new_input_bias.append(new_in_b)

        sh = newdna_in_w1.size + new_in_b.size
        newdna_in_w2 = np.array([newdna[sh:sh
                                + generations[2][0].size]])
        new_hid_w = np.reshape(newdna_in_w2, (-1,
                               generations[2][0].shape[1]))
        new_hidden_weight.append(new_hid_w)

        sl = newdna_in_w1.size + new_in_b.size + newdna_in_w2.size
        new_out_w = np.array([newdna[sl:]]).T
        new_out_w = np.reshape(new_out_w, (-1,
                               generations[3][0].shape[1]))
        new_output_weight.append(new_out_w)

        new_award = run_env(env, new_in_w, new_in_b, new_hid_w,
                            new_out_w)  # bias
...
def evolution(env, test_run, n_of_generations):
    ...
    for n in range(n_of_generations):
        (new_generation, new_award_set) = reproduce(current_award_set,
                current_gens)
        current_gens = new_generation
        current_award_set = new_award_set
        avg = np.average(current_award_set)
        best_gen = np.array([current_gens[0][0], current_gens[1][0],
                            current_gens[2][0], current_gens[3][0]])
        np.save('newtest', best_gen)
        a = np.amax(current_award_set)
        print('generation: {}, max reward: {}, gamescore:{}'.format(n + 1, a, env.get_score()))
        A = np.append(A, a)
