...
class EnemyShip(MovingObject):
    ...
    def __init__(self, sensitivity: int, initial_x: int, initial_y: int, lives: int, game_width: int, game_height: int,
                 switch_direction: Direction, bullet_grp, debug: bool):
        """Constructor for the enemy ship"""

        # Call the superclass
        super().__init__(sensitivity, initial_x, initial_y, game_width, game_height,
                         self.sprites[lives - 1 if lives <= len(self.sprites) else len(self.sprites) - 1], debug)
