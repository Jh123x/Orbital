...
class Stage2Screen(StoryTemplate):
    ...
    def __init__(self, screen_width:int, screen_height:int, screen, sensitivity:int, max_fps:int, tracker:AchievmentTracker, debug:bool):
        ...
        super().__init__(screen_width, screen_height, screen, State(101), sensitivity, max_fps, 0.1,tracker, debug)
        ...
        self.tonald_drump = ImageObject(self.screen_width//2, int(self.screen_height * 210 / 800), 217, 217, StoryTemplate.sprites_dict['drump'], debug)
        self.tonald_drump.scale(int(217 * screen_width//600),int(217 * screen_height//800))
    ...
    def update_trackers(self, win = False):
        super(Stage2Screen, self).update_trackers()
        if win:
            self.tracker.add_value('st_2_clr', 1)
