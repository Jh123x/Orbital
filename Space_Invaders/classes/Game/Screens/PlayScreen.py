...
class PlayScreen(ClassicScreen):
    def __init__(self, screen_width: int, screen_height: int, screen, sensitivity: int, max_fps: int,
        ...
        super().__init__(screen_width, screen_height, screen, sensitivity, max_fps, difficulty, tracker, wave,
                         player_lives, debug)
    ...
    def spawn_brute(self) -> None:
        ...
        if self.wave % 15 == 0:
            ...
            self.other_enemies.add(
                Brute(self.sensitivity, self.generate_random_no() * self.screen_width, self.screen_height // 10,
                      self.screen_width, self.screen_height, self.mob_bullet, self.debug))
    ...
    def spawn_scout(self) -> None:
        ...
        if self.wave % 5 == 0:
            # Add the scout to the other enemies grp
            self.other_enemies.add(
                Scout(self.sensitivity, self.screen_width // 4 + self.screen_width // 10, self.screen_height // 10, 1,
                      self.screen_width, self.screen_height, self.mob_bullet, self.debug))
