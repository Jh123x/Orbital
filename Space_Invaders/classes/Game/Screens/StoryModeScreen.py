...
class StoryModeScreen(MenuTemplate):
    ...
    def create_videos(self) -> None:
        ...
        for i in range(1,self.get_stages()//2+1):
            ...
            self.video_buttons.append(self.write(self.end_font, WHITE, f"Stage {i}   ", self.screen_width // 2 - 40, self.screen_height // 2 + (40*i) - 30, Direction.RIGHT))

        for i in range(1, self.get_stages()  - self.get_stages()//2 + 1):

            #Append the button to the buttons list for checking later
            self.video_buttons.append(self.write(self.end_font, WHITE, f" Stage {self.get_stages()//2 + i}", self.screen_width // 2 + 40, self.screen_height // 2 + (40*i - self.get_stages() // 2 + 1) - 30, Direction.LEFT))
