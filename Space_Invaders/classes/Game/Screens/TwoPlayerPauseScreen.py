...
class TwoPlayerPauseScreen(PauseScreen):
    ...
    def __init__(self, screen_width: int, screen_height: int, screen, player_scores: tuple, previous_state: State,
                 debug: bool = False):
    ...
    def write_lines(self):
        ...
        self.write(self.end_font, WHITE, f"Player 1: {self.p1_score}", self.screen_width // 4, first_pixel,
                   Direction.LEFT)

        # Draw the player 2 score
        self.write(self.end_font, WHITE, f"Player 2: {self.p2_score}", self.screen_width // 4,
                   first_pixel + self.screen_height // 15, Direction.LEFT)

        # Draw the instructions to unpause
        self.unpause = self.write(self.end_font, WHITE, "Unpause (P)", self.screen_width // 4,
                                  first_pixel + self.screen_height // 7.5, Direction.LEFT)

        # Draw the instructions to quit
        self.quit = self.write(self.end_font, WHITE, "Quit (Esc)", self.screen_width // 4,
                               first_pixel + self.screen_height // 5, Direction.LEFT)
