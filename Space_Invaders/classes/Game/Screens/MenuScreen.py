...
class MenuScreen(MenuTemplate):
    ...
    def write_lines(self) -> None:
        ...
        self.rect_highscore = self.write(self.end_font, WHITE, "High Score", self.screen_width // 2,
                                         self.screen_height // 15 + self.screen_height // 2)

        # Draw the instructions button
        self.rect_instruction = self.write(self.end_font, WHITE, "Instructions", self.screen_width // 2,
        ...
        self.stats = self.write(self.end_font, WHITE, "Stats", self.screen_width // 2,
                                self.screen_height // 5 + self.screen_height // 2 + self.screen_height // 15)
    ...
    def get_rects(self):
        ...
        return (
        self.rect_play, self.rect_highscore, self.rect_instruction, self.rect_settings, self.stats, self.rect_end)

    def get_effects(self):
        """Get the effects for Menu Screen"""
        return (
        State.PLAYMODE, State.HIGHSCORE, State.INSTRUCTIONS_MENU, State.SETTINGS, State.STAT_MENU_SCREEN, State.QUIT)
