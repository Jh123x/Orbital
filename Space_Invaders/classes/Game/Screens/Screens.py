...
class Screen(BaseObject):
    ...
    def __init__(self, screen_width: int, screen_height: int, state: State, screen, initial_x: int, initial_y: int,
                 debug: bool = False):
        """Base Screen object"""
    ...
    def write_main(self, font_type, color: Color, words: str, xpos: int, ypos: int,
                   direction: Direction = Direction.CENTER):
        """Write to main surface directly"""
        return self.write(font_type, color, words, xpos, ypos, direction, self.screen)

    def write(self, font_type, color: Color, words: str, xpos: int, ypos: int, direction: Direction = Direction.CENTER,
              screen=None) -> pygame.Rect:
