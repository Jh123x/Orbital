...
class GameWindow(object):
    def __init__(self, sensitivity: int, maxfps: int, game_width: int, game_height: int, icon_img_path: str,
                 player_img_paths: tuple,
                 enemy_img_paths: tuple, bullet_img_paths: tuple, background_img_paths: tuple,
                 explosion_img_paths: tuple,
                 db_path: str, sound_path: dict, bg_limit: int, menu_music_paths: tuple, powerup_img_path: tuple,
                 mothership_img_path: tuple,
                 trophy_img_path: tuple, scout_img_path: tuple, brute_img_path: tuple, screenshot_path: str,
                 story_img_path: str, crabs_img_path: str,
        ...
        load_sprites((Player, Bullet, EnemyShip, Background, Explosion, MotherShip, VictoryScreen, Scout, Brute, Crabs),
                     (player_img_paths, bullet_img_paths, enemy_img_paths, background_img_paths, explosion_img_paths,
                      mothership_img_path, trophy_img_path, scout_img_path, brute_img_path, crabs_img_path))
        ...
        # Load sounds
        self.sound = asyncio.run(
            load_sound(sound_path, self.settings_data['music'], float(self.settings_data['volume']), self.debug))
        ...
        self.screens = {
            ...
            State.POWERUP_INSTRUCTIONS: PowerupInstructionsScreen(game_width, game_height, self.main_screen, self.fps,
                                                                  debug),
            State.AI_VS: AIPVPScreen(game_width, game_height, self.main_screen, sensitivity, maxfps,
                                     self.achievementtracker, 3, debug),
            State.TUTORIAL: TutorialScreen(game_width, game_height, self.main_screen, sensitivity, maxfps,
                                           self.achievementtracker, debug),
            State.STAGE1: Stage1Screen(game_width, game_height, self.main_screen, sensitivity, maxfps,
                                       self.achievementtracker, debug),
            State.STAGE2: Stage2Screen(game_width, game_height, self.main_screen, sensitivity, maxfps,
                                       self.achievementtracker, debug),
            State.STAGE3: Stage3Screen(game_width, game_height, self.main_screen, sensitivity, maxfps,
                                       self.achievementtracker, debug),
            State.STAGE4: Stage4Screen(game_width, game_height, self.main_screen, sensitivity, maxfps,
                                       self.achievementtracker, debug),
            State.STAGE5: Stage5Screen(game_width, game_height, self.main_screen, sensitivity, maxfps,
                                       self.achievementtracker, debug),
            State.STAGE6: Stage6Screen(game_width, game_height, self.main_screen, sensitivity, maxfps,
                                       self.achievementtracker, debug),
    ...
    def handle_victory(self) -> State:
        ...
        if not self.screens[State.VICTORY] or self.screens[State.VICTORY].get_stage_name() != cleared_stage:
            ...
            self.screens[State.VICTORY] = VictoryScreen(self.game_width, self.game_height, self.main_screen,
                                                        cleared_stage, self.sound)
    ...
    def handle_online(self) -> State:
        ...
        self.popup = Popup(320, 40, "Under Construction", self.fps, self.game_width // 2 - 80, self.game_height // 2,
                           self.main_screen, font=Screen.end_font, debug=self.debug)
    ...
    def _handle_pause(self, pause_state: State, screen_type):
        ...
        if not self.screens[pause_state] or prev_screen.comparator() != self.screens[pause_state].comparator():
            ...
            self.screens[pause_state] = screen_type(self.game_width, self.game_height, self.main_screen, comp,
    ...
    def handle_newhighscore(self) -> State:
        ...
        if not self.screens[State.NEWHIGHSCORE] or self.screens[State.NEWHIGHSCORE].get_score() != self.screens[
            State.PLAY].get_score():
            # Create the new highscore screen
            self.screens[State.NEWHIGHSCORE] = NewhighscoreScreen(self.game_width, self.game_height, self.main_screen,
                                                                  self.screens[State.PLAY].get_score())
        ...
        if state == State.GAMEOVER:
            ...
            self.screens[State.HIGHSCORE].update_score(self.screens[State.NEWHIGHSCORE].get_name(),
                                                       self.screens[State.PLAY].get_score())
    ...
    def _handle_gameovers(self, gameover_state: State, screen_type) -> State:
        ...
        if not self.screens[gameover_state] or self.screens[gameover_state].comparator() != comp or self.screens[
            gameover_state].get_prev_state() != self.prev:
            # Create a new gameover screen
            self.screens[gameover_state] = screen_type(self.game_width, self.game_height, self.main_screen, comp,
                                                       self.prev, self.debug)
    ...
    def check_keypresses(self) -> None:
        ...
        if keys[K_x] and self.state != State.NEWHIGHSCORE:
            ...
            self.popup_q.append(
                Popup(20 * 8, 30, "Screenshot taken", self.fps, self.game_width // 2, 15, self.main_screen,
                      font=Screen.font, debug=self.debug))
